					...
					<el-col :span="4">
						<el-form-item>
							<el-select v-model="form.searchType" placeholder="请选择">
								...
								<el-option label="业务员" value="petname"></el-option>
				...
				<el-table :data="tableData3" border height="460" @selection-change="handleSelectionChange">
					...
					<el-table-column  label="操作" width="calc(100% - 1085px)">
						<template scope="scope">
							<el-button  type="text" size="small" @click="invite_modifu(scope.$index, scope.row)">修改</el-button>
							<!-- <el-button type="text" size="small" @click="handleEdit(scope.$index, scope.row)">禁用</el-button> -->
							<el-button  type="text" size="small" @click="invite_detail(scope.$index, scope.row)">查看详情</el-button>
						</template>
					</el-table-column>
				</el-table>
...
<script>
	...
	export default {
		...
		data() {
			return {
				...
				table_list: [
					...
					{name:'客户名', 	prop:'name',	width:'120'},
					{name:'客户公司',	prop:'companyname',	width:'180'},
					{name:'客户手机号',	prop:'phone',	width:'120'},
					{name:'业务员',	prop:'petname',	width:'100'},
					{name:'课程名称',	prop:'coursename',	width:'150'},
					{name:'课程地点',	prop:'courseaddress',	width:'150'},
					{name:'课程时间',	prop:'coursetime',	width:'150'},
					{name:'应约次数',	prop:'inviteidcount', width:'125'},
		...
		methods: {
			...
			//添加
			add(){
				this.$router.push({path: "/invite_add"})
			},
			//修改
			invite_modifu(index,row){
				this.$router.push({path:"/invite_update",query:{inviterecordid:row.inviterecordid}})
			},
			//查看详情
			invite_detail(index,row){
				this.$router.push({path:"/invite_detail",query:{inviterecordid:row.inviterecordid}})
			},
			//加载列表数据
			loadData(){
					if(this.form.searchType == 'customerName'){
						this.customerName = this.form.searchTypeValue;
					}if(this.form.searchType == 'companyname'){
						this.companyname = this.form.searchTypeValue;
					}if(this.form.searchType == 'petname'){
						this.petname = this.form.searchTypeValue;
					}if(this.form.searchType == 'coursename'){
						this.coursename = this.form.searchTypeValue;
					}
				this.$http.get('/list/inviteRecord/inviteRecordIndex',{
					params: {
        ...
        if(newVal[8].childList[1].childstatus == 0) {
          this.$router.push({path: "/login"})
        }
				this.limitCtrl =newVal[8].childList[1].map;
