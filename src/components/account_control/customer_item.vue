        ...
        <el-form ref="form" :inline="true" :model="form" label-width="80px">
          <el-row>
           ...
           <!--   <el-button  type="success" icon="search" @click="search">查找</el-button>-->
              <el-button  type="info" icon="plus" @click="add">添加信息</el-button>
            </el-form-item>
          </el-row>
        </el-form>
        <!--表格块-->
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        ...
        table_list: [
          ...
          {name: '项目类型', prop: 'itemtypename', width: '120'},
          {name: '帐号类型', prop: 'accountypename', width: '150'},
          {name: '帐号', prop: 'username', width: '200'},
          {name: '密码', prop: 'password', width: '150'},
          {name: '网址', prop: 'url', width: '200'},
          {name: '备注', prop: 'comments', width: '200'},
        ],
        tableData3: [],
        multipleSelection: [],
        //定义列表总数
    ...
    methods: {
      ...
      add() {
      		this.$router.push({path: '/itemAccountNumber_add',query:{custid: this.$route.query.custid, custuserid: this.$route.query.custuserid}})
      },
      //加载列表数据
      loadData() {
//      this.loadDefault();
        ...
        this.$http.get('/list/itemAccountNumberController/findAccountListByCusterid', {
            params: {
