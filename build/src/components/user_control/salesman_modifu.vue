...
<script>
	...
	export default {
		...
		methods: {
			...
			submitForm1(formName) {
				this.$refs[formName].validate((valid) => {
					if(valid) {
						this.$http.post('/list/salesman/update',{
			...
			loadData(){
				this.$http.get('/list/salesman/findSaleBySalephone',{
					...
					this.ruleForm1 = json;
					this.ruleForm1.salephone = Number.isInteger(this.ruleForm1.salephone)? this.ruleForm1.salephone : parseInt(this.ruleForm1.salephone);
				})
			},
			//提示信息
      ...
      limititem(val) {
        ...
        if (newVal[0].childList[1].map.updateFun == 0) {
