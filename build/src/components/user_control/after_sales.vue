    ...
    <main_component @limititem="limititem">
      ...
      <div class="table_list">
        ...
        <el-form ref="form" :inline="true" :model="form" label-width="80px">
          ...
          <el-form-item>
            <el-col :span="11">
              <el-date-picker type="date" placeholder="开始时间" :picker-options="pickerOptions0" v-model="form.startTime"
                              style="width: 100%;"></el-date-picker>
            </el-col>
            <el-col class="line" :span="1">&nbsp;--</el-col>
            <el-col :span="11">
              <el-date-picker type="date" placeholder="结束时间" :picker-options="pickerOptions0" v-model="form.endTime"
                              style="width: 100%;"></el-date-picker>
            </el-col>
          </el-form-item>
          </el-form-item>
        ...
        <el-table :data="tableData3" border height="460" :row-class-name="tableRowClassName" @selection-change="handleSelectionChange">
          ...
          <el-table-column label="操作" width="calc(100% - 975px)">
            <template scope="scope">
              <el-button v-if="limitCtrl.afterSaleFun==1" type="text" size="small"
                         @click="after_sale_log(scope.$index, scope.row)">售后日志
              </el-button>
              <el-button v-if="scope.row.replystatus==1" type="text" size="small"
                         @click="modifu(scope.$index, scope.row)">回复
              </el-button>
            </template>
          </el-table-column>
        </el-table>
    ...
    data() {
      ...
      return {
        ...
        table_list: [
          ...
          {name: '项目', prop: 'itemname', width: '150'},
          {name: '拜访客户', prop: 'contact', width: '120'},
          {name: '客户手机号', prop: 'phone', width: '150'},
          {name: '服务次数', prop: 'visitcount', width: '100'},
          {name: '业务员', prop: 'salename', width: '120'},
          {name: '最近联系时间', prop: 'createdate', width: '180'},
          {name: '售后形式', prop: 'visitwayname', width: '125'},
          {name: '回访人', prop: 'petname', width: '125'},
          {name: '备注', prop: 'visitremarks', width: '250'},
            {name: '是否业务回复', prop: 'replystatusname', width: '150'},
          {name: '业务员回复内容', prop: 'replycontext', width: '250'},
    ...
    methods: {
     ...
     /* add() {
      ...
      modifu(index, row) {
        ...
        this.$router.push({path: "/after_sales_modifu", query: {visitid: row.visitid}})
      },
      //售后日志
      after_sale_log(index, row) {
          this.$router.push({path: "/after_sales_log", query: {itemid: row.itemid}})
      },
      ...
      limititem(val) {
        ...
        if (newVal[0].childList[2].childstatus == 0) {
          this.$router.push({path: "/login"})
        }
        this.limitCtrl = newVal[0].childList[2].map;
      }
    },
  }
</script>
