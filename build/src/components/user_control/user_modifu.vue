					...
					<el-row>
						...
						<el-col :span="18">
							<el-form-item label-width="100px">
								<el-input type="textarea" disabled=true placeholder="填写客户提交的附加资料，及需求描述" :rows='5' v-model="ruleForm1.itemrequire"></el-input>
							</el-form-item>
							<span v-for="(item,index) in itemrequire">
								<el-form-item label-width="100px">
									<el-input type="textarea" disabled :rows='5' v-model="itemrequire[index]"></el-input>
								</el-form-item>
							</span>
						</el-col>
						<!--添加-->
						<!--<el-col v-if=isAddRequire :span="18">
							<el-form-item label-width="100px">
								<el-input type="textarea" placeholder="填写客户提交的附加资料，及需求描述" :rows='5' v-model="ruleForm1.itemrequire"></el-input>
							</el-form-item>
						</el-col>
						<el-col :span="18"  :offset="16"><el-button icon="plus"  @click="isAddInput()">添加新的需求</el-button></el-col>-->
						<el-col :span="18"  :offset="16"><el-button icon="plus" type="primary" @click="isAddRequireVisibleAdd(1)">添加新的需求</el-button></el-col>
								<el-dialog title="添加需求" :visible.sync="isAddRequireVisible" :modal-append-to-body="false">
									<el-input type="textarea" placeholder="填写客户提交的附加资料，及需求描述" :rows='5' v-model="Additemrequire"></el-input>
									<el-form-item label-width="100px">
										<el-upload class="upload-demo"  :data="token" action="/list/customer/uploadRequireDoc" :on-success="handlesuccess1" :on-preview="handlePreview" :file-list="fileList4" :on-remove="handleRemove1" :before-upload="beforeAvatarUpload1">
											<el-button size="small" type="primary"  @click="change_salesman()">上传需求文档</el-button>
											&nbsp;
											<i slot="tip" class="el-upload__tip">上传word、pdf、ececl格式</i>
										</el-upload>
									</el-form-item>
									<div slot="footer" class="dialog-footer">
										<el-button @click="isAddRequireVisibleAdd(0)">取 消</el-button>
										<el-button type="primary" @click="isAddRequireVisibleAdd(2)">确 定</el-button>
									</div>
								</el-dialog>
						<el-col :span="18">
							<el-form-item label-width="100px">
								<el-upload class="upload-demo" :disabled=true :data="token" action="/list/customer/uploadRequireDoc" :on-success="handlesuccess1" :on-preview="handlePreview" :file-list="fileList3" :on-remove="handleRemove1" :before-upload="beforeAvatarUpload1">
									<el-button size="small" type="primary" disabled=true  @click="addRequireChange(1)">上传需求文档</el-button>
									&nbsp;
									<i slot="tip" class="el-upload__tip">上传word、pdf、ececl格式</i>
								</el-upload>
							</el-form-item>
	...
	export default {
		...
		data() {
			...
			var checkDate1 = (rule, value, callback) => {
				...
				}else if (value <= this.changeTime(this.ruleForm1.itembegindate)) {
		...
		methods: {
			...
			this.$http.get("/list/customer/update/index", {
				...
				if(res.body.data != null && res.body.code == 100) {
					...
					this.ruleForm1.itemtypedetail = res.body.data.itemtypedetail == 0? 1: res.body.data.itemtypedetail;
			...
			addRequireChange(changeType,requireid){
				this.$http.get("/list/requireChange/add",{
					params: {
						token: localStorage.CRM_token,
    					itemid: this.itemid,
    					requireid:requireid,
			...
			isAddRequireVisibleAdd(type) {
				switch(type) {
					...
					case 2:
						this.$http.post('/list/itemRequire/add', {
							token: localStorage.CRM_token,
							itemid: this.itemid,
							itemdocUrl2: this.itemdocUrl2,
							requirecontent: this.Additemrequire
						}, {
							emulateJSON: true
						}).then(function(res) {
							if(res.body.code == 100) {
			...
			handleRemove1(file) {
				...
				console.log("--------------")
				console.log(file)
				console.log(this.itemdocUrl2)
				var flag=false
				for (var i = 0; i < this.itemdocUrl2.length; i++) {
					if(this.itemdocUrl2[i]==file.response.data.docurl){
						flag=true;
					}
				...
				if(flag){
					if(file.response.data != null && file != null) {
						...
						this.deleteData(this.itemdocUrl2, url)
						//删除服务器中的图片
						this.$http.get("/list/customer/realdelete", {
							params: {
								token: localStorage.CRM_token,
