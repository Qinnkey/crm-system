					...
					<el-row>
						...
						<el-col :span="9">
							<el-form-item label="总金额：" prop="allmoney">
								<el-input type="tel" v-model.number="ruleForm2.allmoney" auto-complete="off"></el-input>
							</el-form-item>
						</el-col>
						<el-col :span="9">
							<el-form-item label="开发费：" prop="developmoney">
								<el-input type="tel" v-model.number="ruleForm2.developmoney" auto-complete="off"></el-input>
							</el-form-item>
						</el-col>
						<el-col :span="9">
							<el-form-item label="服务费：" prop="servicemoney">
								<el-input type="tel" v-model.number="ruleForm2.servicemoney" auto-complete="off"></el-input>
							</el-form-item>
						</el-col>
		...
		methods: {
			...
			submitForm(formName) {
				this.$refs[formName].validate((valid) => {
					if(valid) {
						var url = '/list/customer/updateItemPo';
						var json = {token: localStorage.CRM_token,itemid:this.ruleForm2.itemid,allmoney:this.ruleForm2.allmoney,developmoney:this.ruleForm2.developmoney, servicemoney:this.ruleForm2.servicemoney};
						console.log(json);
						this.postSubmit(url,json);
						this.$router.go(0);
					} else {
			...
			resetForm(formName) {
      		...
      		postSubmit(url,json){
      			console.info(json);
      			this.$http.post(url,json,{emulateJSON:true}).then(function(res){
      			})
      		},
      		back(){
      			if (this.flag==1) {
      				this.$router.push({path:'/trade_list'})
      			}else{
      				this.$router.push({path:'/trade_audit_list'})
      			}
      		},
      //权限改变
      limititem(val) {
