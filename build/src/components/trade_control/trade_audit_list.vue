    ...
    <main_component @limititem="limititem">
      ...
      <div class="table_list">
        ...
        <el-table :data="tableData3" border style="width: 100%" height="460" @selection-change="handleSelectionChange">
          ...
          <el-table-column v-if="limitCtrl.detailFun==1" fixed="right" label="操作" width="200">
            <template scope="scope">
              <el-button type="text" size="small" @click="handleEdit(scope.$index, scope.row)">查看详情</el-button>
            </template>
          </el-table-column>
        </el-table>
        <!--续费 提交-->
  ...
  export default {
    ...
    data() {
      return {
        ...
        table_list: [
          ...
          {name: '审核结果', prop: 'auditstatusname', width: '120'},
          {name: '交易凭证号', prop: 'voucherno', width: '160'},
        ],
        tableData3: [],
        multipleSelection: [],
        //定义列表总数
    ...
    methods: {
      ...
      handleEdit(index, row) {
        this.$router.push({path: 'trade_audit_list/detail', query: {itemid: row.itemid}});
      },
      onSubmit() {
      ...
      limititem(val) {
        ...
        if (newVal[1].childList[1].childstatus == 0) {
          this.$router.push({path: "/login"})
        }
        this.limitCtrl = newVal[1].childList[1].map;
      }
    }
  }
</script>
